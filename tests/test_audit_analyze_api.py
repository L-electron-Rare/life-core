...
class _FakeAnalysisError(RuntimeError):
    """Fake analyzer error used to avoid importing makelife in tests."""


def _mock_analyzer(*, single_result=None, cross_result=None, single_side_effect=None, cross_side_effect=None):
    """Context manager that mocks the lazy analyzer loader."""

    class _FakeAuditAnalyzer:
        def __init__(self, model: str):
            self.model = model

        def analyze_single(self, file_path: str):
            if single_side_effect is not None:
                raise single_side_effect
            return single_result

        def analyze_cross(self, file_paths: list[str]):
            if cross_side_effect is not None:
                raise cross_side_effect
            return cross_result

    return patch(
        "life_core.audit_analyze_handler._load_audit_analyzer",
        return_value=(_FakeAuditAnalyzer, _FakeAnalysisError),
    )
...
def test_analyze_single_returns_200(client, tmp_path):
    ...
    with _mock_analyzer(single_result=MOCK_ANALYSIS):
...
def test_analyze_cross_returns_200(client, tmp_path):
    ...
    with _mock_analyzer(cross_result=cross_result):
        response = client.post(
            "/audit/analyze",
...
def test_analyze_file_not_found_returns_404(client):
    with _mock_analyzer(single_side_effect=FileNotFoundError("not found")):
        response = client.post(
            "/audit/analyze",
...
def test_analyze_llm_error_returns_502(client, tmp_path):
    ...
    with _mock_analyzer(single_side_effect=_FakeAnalysisError("LLM unavailable")):
        response = client.post(
            "/audit/analyze",
            json={"file_path": str(audit_file)},
        )
...
def test_analyze_dependency_missing_returns_503(client, tmp_path):
    ...
    with patch(
        "life_core.audit_analyze_handler._load_audit_analyzer",
        side_effect=AuditAnalyzerUnavailableError("makelife package not found"),
    ):
        response = client.post(
            "/audit/analyze",
            json={"file_path": str(audit_file)},
    ...
    assert "makelife package not found" in response.json()["detail"]
